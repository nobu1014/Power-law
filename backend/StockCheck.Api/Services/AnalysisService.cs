...
using StockCheck.Api.Models.Responses;   // ← これが入っているか確認
...
/// 株価分析サービス
/// ・入力正規化
/// ・watchlist / symbols 登録保証（ユーザーIDに紐づけ）
/// ・DB から取得したデータを画面表示用レスポンスに組み立てる
/// ・外部API取得は ImportService に委譲
/// </summary>
public class AnalysisService
{
    ...
    public async Task<AnalysisResponse> AnalyzeAsync(AnalysisRequest request)
    {
        ...
        // 分析対象銘柄をユーザーのwatchlist / symbolsに登録する
        await _watchlistRepository.AddAsync(request.UserId, symbol, market);
        ...
        CancellationToken ct)
    {
        ...
        // ユーザーIDに紐づけてwatchlist / symbolsに登録する
        await _watchlistRepository.AddAsync(userId, symbol, market);
        ...
        var previousBusinessDay = GetPreviousBusinessDay(DateTime.Today);
    ...
    /// <summary>
    /// 夜間バッチ時間帯かどうかを判定する（2:00〜4:00はAPI取得禁止）
    /// </summary>
